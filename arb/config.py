...
class ArbitrageConfig:
    ...
    def validate(self) -> None:
        ...
        if not 0 < self.min_roi_pct < 1:
            errors.append(
                f"min_roi_pct must be between 0 and 1, got {self.min_roi_pct}"
            )

        if self.min_liquidity_usd <= 0:
            errors.append(
                f"min_liquidity_usd must be positive, got {self.min_liquidity_usd}"
            )
        ...
        if self.max_concurrent_spreads <= 0:
            errors.append(
                f"max_concurrent_spreads must be positive, got {self.max_concurrent_spreads}"
            )

        if self.scan_interval_seconds <= 0:
            errors.append(
                f"scan_interval_seconds must be positive, got {self.scan_interval_seconds}"
            )
        ...
        if not 0 < self.max_error_rate < 1:
            errors.append(
                f"max_error_rate must be between 0 and 1, got {self.max_error_rate}"
            )

        if not 0 < self.min_fill_rate <= 1:
            errors.append(
                f"min_fill_rate must be between 0 and 1, got {self.min_fill_rate}"
            )
    ...
    def from_dict(cls, data: Dict) -> "ArbitrageConfig":
        ...
        return cls(
            ...
            min_edge_cents_conservative=float(
                data.get("min_edge_cents_conservative", 7.0)
            ),
            ...
            use_conservative_fees_for_filtering=bool(
                data.get("use_conservative_fees_for_filtering", True)
            ),
            ...
            reconciliation_interval_seconds=float(
                data.get("reconciliation_interval_seconds", 60.0)
            ),
