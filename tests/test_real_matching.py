...
from src.matching.normalizer import TextNormalizer
...
def print_market_sample(markets, platform_name, n=5):
    ...
    for i, m in enumerate(markets[:n]):
        if platform_name == "Kalshi":
            ...
            print(f"   Category: {m.get('category', 'N/A')}, Close: {m.get('close_time', 'N/A')}")
        else:
            print(f"{i+1}. [{m.get('token_id', 'N/A')[:20]}...] {m.get('question', 'N/A')}")
...
def run_matching(kalshi_markets, poly_markets):
    ...
    for m in matches[:10]:
        ...
        print(f"  Score breakdown: entity={m.entity_score:.2f}, text={m.text_score:.2f}, "
