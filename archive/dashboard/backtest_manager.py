...
class BacktestManager:
    ...
    PERSIST_DIR = Path("data/backtests")
    ...
    def _load_persisted_jobs(self) -> None:
        """Load completed backtest results from disk."""
        ...
        for path in self.PERSIST_DIR.glob("*.json"):
            try:
                ...
                config = BacktestConfig(
                    strategy=StrategyType(data["config"]["strategy"]),
                    data_source=data["config"]["data_source"],
                    **{
                        k: v
                        for k, v in data["config"].items()
                        if k not in ("strategy", "data_source")
                    },
                )
                job = BacktestJob(
                    id=data["id"],
                ...
                if data.get("progress"):
                    job.progress = BacktestProgress(
                        frames_processed=data["progress"].get("frames_processed", 0),
                        total_frames=data["progress"].get("total_frames", 0),
                        signals_generated=data["progress"].get("signals_generated", 0),
                        trades_executed=data["progress"].get("trades_executed", 0),
                        current_pnl=data["progress"].get("current_pnl", 0.0),
                    )
                self._jobs[job.id] = job
                loaded += 1
            except Exception as e:
                logger.warning("Failed to load backtest %s: %s", path.name, e)

        if loaded:
            logger.info("Loaded %d persisted backtest results", loaded)

    def _persist_job(self, job: BacktestJob) -> None:
        """Save a completed backtest job to disk."""
        if job.status not in (BacktestStatus.COMPLETED, BacktestStatus.FAILED):
            return

        try:
            self.PERSIST_DIR.mkdir(parents=True, exist_ok=True)
            path = self.PERSIST_DIR / f"{job.id}.json"
            ...
            logger.warning("Failed to persist backtest %s: %s", job.id, e)
    ...
    def delete_job(self, job_id: str) -> bool:
        ...
        with self._lock:
            ...
            if job.status in (
                ...
                persist_path = self.PERSIST_DIR / f"{job_id}.json"
                if persist_path.exists():
                    try:
                        persist_path.unlink()
                    except Exception as e:
                        logger.warning(
                            "Failed to delete persisted backtest %s: %s", job_id, e
                        )
                return True
    ...
    async def run_job(self, job_id: str) -> None:
        ...
        try:
            ...
            # Persist completed/failed results
    ...
    async def _run_nba_backtest_with_progress(
        ...
        async for frame in backtester.replay.run(backtester.mock_client):
            ...
            if frame_count % 5 == 1 or frame_count == 1:
                home_mid = (
                    (frame.home_bid + frame.home_ask) / 2
                    if frame.home_bid and frame.home_ask
                    else 0
                )
            ...
            if frame_count % 50 == 0 or frame_count == len(recording.frames):
                ...
                current_pnl = sum(
                    s.theoretical_pnl or 0 for s in backtester.signals if s.filled
                )
                ...
                job.progress.trades_executed = sum(
                    1 for s in backtester.signals if s.filled
                )
