...
class TestAPIFailures:
    ...
    def test_api_timeout_handled(self) -> None:
        ...
        # Mock API to timeout
        with patch.object(
            engine.quote_manager, "place_quote", side_effect=TimeoutError("API timeout")
        ):
            # Should not crash
            engine.on_market_update(market)
    ...
    def test_api_error_handled_gracefully(self) -> None:
        ...
        # Mock API to fail
        with patch.object(
            engine.quote_manager, "place_quote", side_effect=Exception("API error")
        ):
