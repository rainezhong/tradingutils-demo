...
try:
    from src.core import Config, MarketDatabase, get_config, setup_logger
except ImportError:
    Config = None
    MarketDatabase = None
    get_config = None
...
class SystemMonitor:
    ...
    def __init__(self, config: Optional["Config"] = None):
        ...
        self.db = (
            MarketDatabase(self.config.db_path)
            if MarketDatabase and self.config
            else None
        )
    ...
    def get_snapshots_today(self) -> int:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT COUNT(*) as count FROM snapshots WHERE timestamp >= ?", (today,)
        )
    ...
    def get_success_rate(self, hours: int = 24) -> float:
        ...
        cursor.execute(
            "SELECT COUNT(*) as count FROM snapshots WHERE timestamp >= ?", (cutoff,)
        )
