...
from src.core import Config, set_config, setup_logger
...
def cmd_analyze(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if not markets:
            print(
                "No markets in database. Run 'python3 main.py scan' first to populate data."
            )
            return 1
        ...
        if args.strategy:
            ...
            print(
                f"Markets for {args.strategy} (suitability >= {args.min_suitability}):\n"
            )
            print("-" * 90)
            print(
                f"{'Rank':<5} {'Ticker':<25} {'Suitability':<12} {'Score':<8} {'Spread%':<10} {'Volume':<12}"
            )
            print("-" * 90)
            ...
            for idx, row in markets.iterrows():
                ...
                print(
                    f"{idx + 1:<5} {row['ticker']:<25} {suitability:<12.1f} {row['score']:<8.1f} {spread:<10.2f} {volume:<12.0f}"
                )
        ...
        if args.export:
            ...
            if include_strategies:
                print("-" * 110)
                print(
                    f"{'Rank':<5} {'Ticker':<25} {'Score':<8} {'Spread%':<10} {'Volume':<12} {'Best Strategy':<20} {'Suit.':<6}"
                )
                print("-" * 110)
                ...
                for idx, row in top_markets.iterrows():
                    ...
                    print(
                        f"{idx + 1:<5} {row['ticker']:<25} {row['score']:<8.1f} {spread:<10.2f} {volume:<12.0f} {best_strategy:<20} {strategy_score:<6.1f}"
                    )
                ...
                print("-" * 80)
                print(
                    f"{'Rank':<5} {'Ticker':<25} {'Score':<8} {'Spread%':<10} {'Volume':<12}"
                )
                print("-" * 80)
                ...
                for idx, row in top_markets.iterrows():
                    ...
                    volume = row.get("avg_volume", 0) or 0
                    print(
                        f"{idx + 1:<5} {row['ticker']:<25} {row['score']:<8.1f} {spread:<10.2f} {volume:<12.0f}"
                    )
...
def cmd_ncaab_record(args: argparse.Namespace) -> int:
    """Run NCAAB game auto-scheduler."""
    print("=== NCAAB Game Auto-Scheduler ===\n")
    ...
    try:
        ...
        for snap in snapshots:
            ...
            mm_market = MMMarketState(
                ...
                mid_price=snap.mid_price / 100.0
                if snap.mid_price
                else (snap.yes_bid + snap.yes_ask) / 200.0,
...
def main() -> int:
    ...
    scan_parser.add_argument(
        "--min-volume", type=int, default=None, help="Minimum 24h volume"
    )
    scan_parser.add_argument(
        "--min-days", type=int, default=7, help="Minimum days until close"
    )
    ...
    log_parser.add_argument(
        "--bulk",
        action="store_true",
        help="Fast bulk mode - fetches all markets in ~25 API calls",
    )

    # Analyze command
    analyze_parser = subparsers.add_parser("analyze", help="Analyze and rank markets")
    analyze_parser.add_argument(
        "--days", type=int, default=3, help="Days of data to analyze"
    )
    analyze_parser.add_argument(
        "--min-score", type=float, default=12.0, help="Minimum score threshold"
    )
    analyze_parser.add_argument(
        "--top", type=int, default=10, help="Number of top markets to show"
    )
    analyze_parser.add_argument(
        "--export", type=str, default=None, help="Export to CSV file"
    )
    analyze_parser.add_argument(
        "--strategy",
        type=str,
    ...
    sim_parser = subparsers.add_parser(
        "run-simulation", help="Run market-making with simulated data"
    )
    sim_parser.add_argument(
        "--ticker", type=str, default="SIM-MARKET", help="Simulated ticker name"
    )
    sim_parser.add_argument(
    ...
    sim_parser.add_argument(
        "--spread", type=float, default=0.04, help="Target spread (e.g., 0.04 for 4%%)"
    )
    sim_parser.add_argument(
        "--max-position", type=int, default=50, help="Maximum position size"
    )
    sim_parser.add_argument(
    ...
    single_parser = subparsers.add_parser(
        "run-single", help="Run market-making on single market"
    )
    single_parser.add_argument("ticker", type=str, help="Market ticker")
    single_parser.add_argument(
        "--paper",
        action="store_true",
        help="Paper trading mode (simulated data, no API)",
    )
    single_parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Dry run mode (real data, orders logged but not placed)",
    )
    single_parser.add_argument(
        "--spread", type=float, default=0.04, help="Target spread"
    )
    single_parser.add_argument("--size", type=int, default=20, help="Quote size")
    single_parser.add_argument(
        "--max-position", type=int, default=50, help="Maximum position"
    )
    single_parser.add_argument(
        "--max-loss", type=float, default=50.0, help="Max loss per position (dollars)"
    )
    single_parser.add_argument(
        "--daily-loss", type=float, default=200.0, help="Max daily loss (dollars)"
    )

    # run-multi command
    multi_parser = subparsers.add_parser(
        "run-multi", help="Run market-making on multiple markets"
    )
    multi_parser.add_argument(
        "tickers", type=str, help="Comma-separated market tickers"
    )
    multi_parser.add_argument(
        "--paper", action="store_true", help="Paper trading mode (simulated data)"
    )
    multi_parser.add_argument(
        "--dry-run", action="store_true", help="Dry run mode (real data, orders logged)"
    )
    multi_parser.add_argument(
        "--spread", type=float, default=0.04, help="Target spread"
    )
    multi_parser.add_argument(
        "--max-total-position", type=int, default=200, help="Max total position"
    )
    multi_parser.add_argument(
        "--daily-loss", type=float, default=500.0, help="Max daily loss"
    )

    # backtest command
    backtest_parser = subparsers.add_parser(
        "backtest", help="Backtest strategy on historical data"
    )
    backtest_parser.add_argument("ticker", type=str, help="Market ticker")
    backtest_parser.add_argument(
        "--days", type=int, default=7, help="Days of historical data"
    )
    backtest_parser.add_argument(
        "--spread", type=float, default=0.04, help="Target spread"
    )
    ...
    # nba-record command
    nba_parser = subparsers.add_parser(
        "nba-record", help="Auto-detect and record NBA games"
    )
    nba_parser.add_argument(
        "--once", action="store_true", help="Poll once and exit (for testing)"
    )
    nba_parser.add_argument(
        "--poll-interval",
        type=int,
    ...
    # ncaab-record command
    ncaab_parser = subparsers.add_parser(
        "ncaab-record", help="Auto-detect and record NCAAB games"
    )
    ncaab_parser.add_argument(
        "--once", action="store_true", help="Poll once and exit (for testing)"
    )
    ncaab_parser.add_argument(
        "--poll-interval",
    ...
    ncaab_parser.add_argument("--demo", action="store_true", help="Use Kalshi demo API")
    ncaab_parser.add_argument(
        "-v", "--verbose", action="store_true", help="Verbose output"
    )
    ...
    elif args.command == "ncaab-record":
        return cmd_ncaab_record(args)
    else:
        parser.print_help()
        return 0
