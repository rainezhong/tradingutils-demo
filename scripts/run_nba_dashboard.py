...
class MockNBAGame:
    ...
    def __init__(self, game_id: str, home_team: str, away_team: str, home_strength: float = None):
...
def calculate_win_probability(score_diff: int, period: int, time_remaining_sec: int) -> float:
    """
...
class NBAStrategySimulator:
    ...
    def add_game(self, game: MockNBAGame, staleness: float = 0.0):
        ...
        state_aggregator.log_activity(
            ...
            details={"game_id": game.game_id, "market_staleness": f"{staleness*100:.0f}%"}
        )
    ...
    def tick(self, game_seconds: int = 30):
        ...
        for game_id, game in self.games.items():
            ...
            # Advance game clock
            old_score_diff = game.score_differential
            game.tick(game_seconds)

            # Check if score changed significantly
            score_changed = game.score_differential != old_score_diff
            ...
            if has_edge and is_trading_allowed:
                ...
                last_signal = f"BLOCKED Q{game.period} ({abs(edge_cents):.1f}c edge exists)"
            ...
            unrealized_pnl = (yes_value + no_value) - pos.total_cost
    ...
    def _update_total_pnl(self):
        ...
        for game_id, pos in self.positions.items():
            ...
            if game.is_live:
                ...
                market_price = simulate_market_price(true_prob, self.market_staleness[game_id])
        ...
        if current >= 50:
            ...
            if not self._last_logged_position_limit.get(limit_key):
                ...
                state_aggregator.log_activity(
                    ...
                    details={"side": side, "current_position": current, "limit": 50, "note": "Further orders blocked"}
                )
            return
...
def main():
    ...
    games = [
        (MockNBAGame("game_1", "LAL", "BOS", home_strength=0.52), 0.3, "Moderate staleness"),
        (MockNBAGame("game_2", "GSW", "MIA", home_strength=0.55), 0.0, "Efficient market"),
        (MockNBAGame("game_3", "NYK", "CHI", home_strength=0.60), 0.5, "Very stale market"),
    ]
