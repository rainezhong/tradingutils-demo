...
def parse_ticker_teams(ticker: str, title: str, yes_subtitle: str) -> Tuple[str, str, str]:
    """
...
class NBAHistoricalAnalyzer:
    ...
    def fetch_all_settled_markets(self, limit: Optional[int] = None) -> List[Any]:
        ...
        if self.use_cache and cache_file.exists():
            ...
            if cache_age < timedelta(hours=24):
                print(f"Loading markets from cache ({cache_age.seconds // 3600}h old)...")
    ...
    def fetch_game_candlesticks(
        ...
        try:
            ...
            start_ts = int((settlement_time - timedelta(hours=lookback_hours)).timestamp())
    ...
    def aggregate_by_time(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        agg = df.groupby("minutes_until_settlement").agg({
            "favorite_price": "mean",
    ...
    def aggregate_by_price_bucket(
        ...
        for low, high in buckets:
            ...
            rows.append({
                ...
                "calibration_error": subset["favorite_won"].mean() - subset["favorite_price"].mean(),
...
def plot_timeseries(time_agg: pd.DataFrame, season: str = "All Seasons"):
    ...
    ax1 = axes[0, 0]
    ax1.plot(time_agg.index, time_agg["favorite_price"], 'b-', linewidth=2, label='Avg Favorite Price')
    ax1.plot(time_agg.index, time_agg["favorite_won"], 'g--', linewidth=2, label='Actual Win Rate')
    ...
    ax2.bar(time_agg.index, time_agg["calibration_error"] * 100, color=colors, alpha=0.7)
...
def plot_calibration(price_agg: pd.DataFrame, season: str = "All Seasons"):
    ...
    ax1.scatter(price_agg["avg_price"], price_agg["actual_win_rate"], 
                s=price_agg["count"] / 10, alpha=0.7, label='Observed')
    ...
    bars = ax2.bar(price_agg["price_range"], price_agg["avg_ev"] * 100, color=colors, alpha=0.7)
    ax2.axhline(y=0, color='black', linestyle='-', linewidth=1)
...
def print_summary(raw_df: pd.DataFrame, time_agg: pd.DataFrame, price_agg: pd.DataFrame):
    ...
    print(f"Date range: {raw_df['game_date'].min().date()} to {raw_df['game_date'].max().date()}")
    
    print("\n--- Overall Statistics ---")
    print(f"Average favorite price: {raw_df['favorite_price'].mean():.1%}")
    print(f"Favorite win rate: {raw_df['favorite_won'].mean():.1%}")
    print(f"Average EV (betting favorite): {raw_df['ev_favorite'].mean() * 100:.2f} cents/$1")
    
    print("\n--- Calibration by Price Bucket ---")
    print(price_agg[["price_range", "unique_games", "avg_price", "actual_win_rate", "calibration_error", "avg_ev"]].to_string(index=False))
    ...
    for t in key_times:
        if t in time_agg.index:
            row = time_agg.loc[t]
            print(f"  {t:3d} min: Price={row['favorite_price']:.1%}, WinRate={row['favorite_won']:.1%}, EV={row['ev_favorite']*100:+.2f}¢")


def main():
    parser = argparse.ArgumentParser(description="Analyze NBA Historical EV on Kalshi")
    parser.add_argument("--season", type=str, help="Filter to specific season (e.g., '2024-25')")
    parser.add_argument("--limit", type=int, help="Limit number of games to process")
    parser.add_argument("--export", type=str, metavar="FILE", help="Export raw data to CSV")
    parser.add_argument("--no-cache", action="store_true", help="Skip cache, refetch all data")
