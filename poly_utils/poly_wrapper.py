...
try:
    ...
    try:
        from web3.middleware.geth_poa import geth_poa_middleware  # web3 v6
    except ImportError:
...
from poly_utils.objects import SimpleMarket, SimpleEvent
from colorama import Fore, Style

class PolymarketWrapped:
    def __init__(self):
        ...
        self.usdc_address = Web3.to_checksum_address("0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174")  # USDC on Polygon
    ...
    def get_all_markets(self) -> "list[SimpleMarket]":
        markets = []
        # Use events endpoint with closed=false to get current active markets
        res = httpx.get(self.gamma_events_endpoint, params={"closed": "false", "limit": 100})
        if res.status_code == 200:
            for event in res.json():
                try:
                    # Events have nested markets - extract them
                    ...
                    if event_markets:
                        ...
                        # Fallback: treat event as market if no nested markets
                        market_data = self.map_api_to_market(event)
                        markets.append(SimpleMarket(**market_data))
