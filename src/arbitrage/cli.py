...
Usage:
    python -m src.arbitrage.cli start [--paper] [--config config.yaml]
    python -m src.arbitrage.cli stop
    python -m src.arbitrage.cli status
    python -m src.arbitrage.cli positions
    python -m src.arbitrage.cli opportunities [--min-roi 0.05]
    python -m src.arbitrage.cli circuit-breaker status
    python -m src.arbitrage.cli circuit-breaker reset --confirm
"""
...
def start(ctx, paper, daemon):
    ...
    if is_running():
        click.echo("Error: Arbitrage system is already running (PID: {})".format(read_pid()))
    ...
    async def run_checks():
        ...
        if result.passed:
            ...
            for failure in result.failures:
                click.echo(click.style(f"  - {failure.name}: {failure.message}", fg="red"))
