...
class PreflightChecker:
        ...
        for name, check_fn in checks:
            try:
                ...
                if check_result.failed:
                    logger.error("Preflight FAILED: %s - %s", name, check_result.message)
                elif check_result.status == CheckStatus.WARNING:
                    logger.warning("Preflight WARNING: %s - %s", name, check_result.message)
                else:
    ...
    async def _check_kalshi_positions(self) -> CheckResult:
        ...
        try:
            ...
            open_positions = [
                ...
                if hasattr(p, "position") and p.position != 0
                or hasattr(p, "size") and p.size != 0
            ]
            ...
            for pos in open_positions:
                ticker = getattr(pos, "ticker", None) or getattr(pos, "market_ticker", "unknown")
    ...
    async def _check_polymarket_positions(self) -> CheckResult:
        ...
        try:
            ...
            open_positions = [
                o for o in orders
                if hasattr(o, "is_active") and o.is_active
                or hasattr(o, "size_matched") and o.size_matched > 0
            ]
    ...
    async def _check_config(self) -> CheckResult:
        ...
        try:
            ...
            if self._config.max_daily_loss > 1000:
                warnings.append(f"High daily loss limit: ${self._config.max_daily_loss}")

            if self._config.max_concurrent_spreads > 5:
                warnings.append(f"High concurrent spreads: {self._config.max_concurrent_spreads}")
