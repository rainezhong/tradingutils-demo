...
class OpportunityDetector:
    ...
    def scan_single_pair(self, pair: MatchedMarketPair) -> List[RankedOpportunity]:
        ...
        opportunities.extend(
            self._check_cross_platform(pair, p1_yes, p2_yes, "yes")
        )
        opportunities.extend(
            self._check_cross_platform(pair, p2_yes, p1_yes, "yes")
        )
        opportunities.extend(
            self._check_cross_platform(pair, p1_no, p2_no, "no")
        )
        opportunities.extend(
            self._check_cross_platform(pair, p2_no, p1_no, "no")
        )

        # Check dutch book opportunities
        opportunities.extend(
            self._check_dutch_book(pair, p1_yes, p2_no)
        )
        opportunities.extend(
            self._check_dutch_book(pair, p1_no, p2_yes)
        )
    ...
    def _filter_and_rank(
        ...
        for opp in opportunities:
            ...
            limited_opp = SpreadOpportunity(
                ...
                total_fees_per_contract=actual_analysis.total_fees / effective_size if effective_size > 0 else 0,
    ...
    def _check_cross_platform(
        ...
        available_usd = min(buy_quote.ask_depth_usd, sell_quote.bid_depth_usd) * max_depth_pct
        ...
        opp = SpreadOpportunity(
            ...
            total_fees_per_contract=analysis.total_fees / available_contracts if available_contracts > 0 else 0,
    ...
    def _check_dutch_book(
        ...
        available_usd = min(quote_a.ask_depth_usd, quote_b.ask_depth_usd) * max_depth_pct
