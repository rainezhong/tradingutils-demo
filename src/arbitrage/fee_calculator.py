...
class FeeCalculator:
    ...
    def calculate_net_spread(
        ...
        if apply_safety_margin:
            ...
            net_spread = gross_spread - (total_fees / size) if size > 0 else gross_spread
        else:
    ...
    def get_breakeven_spread(
        ...
        gas = self._config.polymarket_gas_estimate if sell_platform == Platform.POLYMARKET else 0
    ...
    def _get_fee_rate(self, platform: Platform, is_maker: bool) -> float:
        ...
        if platform == Platform.KALSHI:
            return self._config.kalshi_maker_fee_rate if is_maker else self._config.kalshi_fee_rate
        else:  # Polymarket
            return self._config.polymarket_maker_fee if is_maker else self._config.polymarket_taker_fee
