...
from ..core import Config, get_config, MarketDatabase, setup_logger
from ..core.trading_state import get_trading_state
from ..collectors import Scanner, Logger
...
class MarketMakerScheduler:
    ...
    def analyze_markets(self) -> dict:
        ...
        try:
            ...
            logger.info(f"Analysis complete: {stats.total_markets} markets, {stats.total_snapshots} snapshots")
            db.close()
            return results
        except Exception as e:
    ...
    def setup_jobs(self) -> None:
        ...
        schedule.every().day.at("06:00").do(self._run_job, self.scan_markets, "scan_markets")
        ...
        schedule.every().day.at("08:00").do(self._run_job, self.analyze_markets, "analyze_markets")
        schedule.every().day.at("20:00").do(self._run_job, self.analyze_markets, "analyze_markets")
    ...
    def _run_job(self, job_func: Callable, job_name: str) -> None:
        ...
        if trading_state.should_pause():
            ...
            if not trading_state.wait_while_paused(timeout=60.0):
                logger.info(f"Skipping job {job_name}: trading still active after timeout")
                return
    ...
    def run_once(self, job_name: str) -> None:
        ...
        if job_name not in jobs:
            raise ValueError(f"Unknown job: {job_name}. Valid jobs: {list(jobs.keys())}")
