...
class MarketMakingEngine:
    ...
    def _force_close_position(self, market: MarketState) -> None:
        ...
        logger.critical(
            f"{self.ticker}: Emergency close - {side} {size}@{price:.4f}"
        )
    ...
    def _should_update_quotes(
        ...
        for new_q in new_quotes:
            matching = [
                q for q in self._last_quotes
                if q.side == new_q.side
            ]
            if not matching:
                return True
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "risk": risk_metrics,
            "market": (
                self._last_market.to_dict() if self._last_market else None
            ),
        }
